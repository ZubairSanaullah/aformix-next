...
export async function POST(_request: NextRequest, context: RouteContext) {
    try {
        ...
        const project = await archivePortfolioProject(parsed.data.id);

        return NextResponse.json(
            {
                message: "Portfolio project archived successfully.",
                project,
            },
            { status: 200 },
        );
        ...
        console.error("[PORTFOLIO_PROJECT_ARCHIVE_API]", error);
