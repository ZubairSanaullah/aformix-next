...
export async function POST(_request: NextRequest, context: RouteContext) {
    try {
        ...
        const project = await trashPortfolioProject(parsed.data.id);

        return NextResponse.json(
            {
                message: "Portfolio project moved to trash.",
                project,
            },
            { status: 200 },
        );
        ...
        console.error("[PORTFOLIO_PROJECT_TRASH_API]", error);
