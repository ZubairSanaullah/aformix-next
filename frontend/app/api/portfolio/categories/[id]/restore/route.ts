...
export async function POST(_request: NextRequest, context: RouteContext) {
    try {
        ...
        const category = await restorePortfolioCategory(parsed.data.id);

        return NextResponse.json(
            {
                message: "Portfolio category restored successfully.",
                category,
            },
            { status: 200 },
        );
        ...
        console.error("[PORTFOLIO_CATEGORY_RESTORE_API]", error);
