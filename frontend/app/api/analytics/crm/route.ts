...
import { getCrmAnalytics } from "@/lib/services/analytics/crm";
...
export async function GET(request: Request) {
    try {
        ...
        const current = await getCrmAnalytics(startDate, endDate);
        let previous = null;
        let comparisons = null;

        if (compare) {
            const { prevStartDate, prevEndDate } = getPreviousPeriod(startDate, endDate);
            previous = await getCrmAnalytics(prevStartDate, prevEndDate);

            comparisons = {
                leads: calculatePercentageChange(current.leads.total, previous.leads.total),
                deals: calculatePercentageChange(current.deals.total, previous.deals.total),
                convertedLeads: calculatePercentageChange(current.leads.converted, previous.leads.converted),
                closedRevenue: calculatePercentageChange(current.deals.closedRevenue, previous.deals.closedRevenue),
            };
        }
        ...
        console.error("[ANALYTICS_CRM_API]", error);
