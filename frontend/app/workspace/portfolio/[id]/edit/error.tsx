...
interface EditPortfolioProjectErrorProps {
    error: Error & { digest?: string };
    reset: () => void;
}

export default function EditPortfolioProjectError({
    error,
    reset,
}: EditPortfolioProjectErrorProps) {
    useEffect(() => {
        console.error(error);
    }, [error]);
        ...
        <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-[var(--workspace-border)] bg-[var(--workspace-surface)] px-6 py-16 text-center">
            ...
            <div>
                ...
                <p className="mt-1 text-sm text-[var(--workspace-text-muted)]">
                    We couldn&apos;t load this portfolio project.
                </p>
            </div>
