...
interface EditKnowledgeArticleErrorProps {
    error: Error & { digest?: string };
    reset: () => void;
}

export default function EditKnowledgeArticleError({
    error,
    reset,
}: EditKnowledgeArticleErrorProps) {
    useEffect(() => {
        console.error("[EDIT_KNOWLEDGE_ARTICLE_ERROR]", error);
    }, [error]);
    ...
    return (
        <div className="flex flex-col items-center justify-center rounded-xl border border-[var(--workspace-border)] bg-[var(--workspace-surface)] px-6 py-16 text-center">
            ...
            <p className="mt-1 max-w-sm text-xs leading-5 text-[var(--workspace-text-muted)]">
                We couldn&apos;t load this article for editing. Please try
